...
Approve every ambiguous observable text behavior, then implement uncached resolved measurement with
one code-point scan, append-only local builders, and linear wrapping/freeze work.

## Context

- M1 supplies trustworthy counters, workload identities, and structural fixtures.
- Existing contradictions are decision gates. This milestone neither silently preserves current
  behavior nor silently changes it.
...
**Purpose:** Turn ambiguous and contradictory behavior into an approved compatibility/migration
matrix before builder implementation.
...
**Key Work:**
- Decide `wordWrap`, replacement-glyph face selection, empty font-chain output, CR/LF handling,
  UTF-16 `charCount`, fallback vertical metrics, and zero/narrow/negative width outcomes.
- Decide exact horizontal/vertical rounding and accumulation order plus caret midpoint tie behavior;
  freeze fixtures before algorithm changes.
- Define line/run/glyph/caret coordinate contracts and the policy for public caret/selection setters
  that receive an index inside a surrogate pair.
- Select line-local cumulative caret advances: width-independent primitives retain raw base advance,
...
- Decide an internal range-aware overload/adapter consumed by M4 without per-range temporary strings
  while preserving current public API compatibility and immutability rules.
- Record source/binary/behavioral compatibility and migration impact for every selected outcome.

**Validation:** Each decision has executable fixture expectations and explicit approval; no P2 task
contains an unresolved compatibility choice.

**Risks / Stop Criteria:** Stop before implementation if any listed decision lacks an owner,
selected behavior, or migration statement.
