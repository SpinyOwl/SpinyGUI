...
**Changes:**
- [x] Implement keys from P1's selected immutable semantic resolved-primitive value key, exact max
...
- [x] Store immutable final `TextMetrics`/line/run and per-final-line cumulative caret arrays with
...
**Acceptance Checks:**
- [x] Every key field changes reuse exactly; exact equal widths hit; no upstream chain/metric/glyph/
  advance/kerning/prepared/primitive family key contains width.
- [x] Many unique widths/large layouts cannot exceed hard retained entry/weight policy.
...
**Changes:**
- [x] Implement P1's selected semantic primitive value-key/value reference direction and shared-object
  weight accounting; never retain cache-entry identity, nodes, or handles.
- [x] Verify primitive cache eviction/clear while wrap entries exist, wrap clear while primitives
  exist, and generation transition behavior.
- [x] Teardown wrapped layouts before primitive/font families and preserve use-after-close/UI-thread
  behavior.

**Acceptance Checks:**
- [x] Clearing either family independently cannot produce dangling access or require an undocumented
  global clear; results are independently immutable/valid or deterministically missed per P1.
- [x] Aggregate weight counts shared source/primitive/result storage exactly as specified.
...
**Changes:**
- [x] Run cold/warm/alternating-width/unique-resize/offset/line-height/vertical-metric/wrap-policy/
  line-breaking/generation/oversized/clear/close/disabled workloads.
- [x] Assert exact line ranges/`charCount`, fallback/replacement runs, cumulative carets, and line-start
  kerning across hits/misses.
- [x] Reconcile wrap hit/miss/eviction/weight with primitive/cache counters and prove disabled invokes
  current M2 wrapping linearly.

**Acceptance Checks:**
- [x] Warm exact keys skip wrap materialization; unique width churn evicts under hard bounds; disabled
  output/counters remain M2-correct and linear.
- [x] No test uses a pre-laid-out renderer scene as the only evidence of wrap-cache behavior.
...
`WrappedLayoutKey`, `WrappedLayoutValue`, and `WrappedLayoutCache` provide exact width/offset/
vertical-metric/wrap-policy keys, immutable line and cumulative-caret values, hard entry/weight
bounds, LRU eviction, owner-thread checks, independent clear/close, and true disabled behavior.
`WrappedLayoutCacheTest` covers exact hits, line-affecting misses, width churn, bounds, and disabled
mode. `FontServiceImpl` now uses the exact wrapped-layout key around the real M2 final materializer;
the cache is cleared before M3 resources during teardown. The standalone value wrapper remains useful
for backend-neutral characterization.
