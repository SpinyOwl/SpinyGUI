...
**Changes:**
- [x] Implement exact semantic font identity/generation/code-point keys and immutable glyph-index
  values that explicitly cache both hit and zero/miss outcomes.
- [x] Apply P1 hard entry/weight/admission/oversized/diagnostics/clear/teardown/disabled policies and
  UI-thread checks.
- [x] Keep logical chain resolution separate: each candidate lookup may hit/miss this native-probe
  family, while logical resolution/replacement policy remains M2 behavior.

**Acceptance Checks:**
- [x] Warm misses avoid native probes under the same generation, and successful semantic mutation
  cannot reuse an old miss.
- [x] Diagnostics separately report logical resolutions, primitive cache hits/negative hits, and
...
**Changes:**
- [x] Implement exact semantic identity/generation/glyph/size/configuration/rounding keys for base
  advances, excluding previous glyph, width, offset, and line state.
- [x] Apply full P1 family policy and immutable numeric value handling, including approved invalid
  numeric behavior.
- [x] Integrate M2 primitive creation so cached base advance remains separable from pair kerning and
  final line materialization.

**Acceptance Checks:**
- [x] Exact equivalent primitives hit; size/configuration/generation changes miss; wrap width/offset
  changes do not affect this family.
- [x] Disabled output/calls match M2 and retained weight remains bounded under glyph/size churn.
...
**Changes:**
- [x] Implement ordered previous/current glyph plus same semantic face/generation/size/configuration
  keys and complete P1 family policy.
- [x] Integrate M2 line materialization so no kerning lookup/contribution is used for a line's first
  primitive or across a selected face transition where M2 resets it.
- [x] Cover zero-kerning results, pair order, cached misses/zeros, generation, and independent clear.

**Acceptance Checks:**
- [x] `(A,V)` and `(V,A)` are distinct; warm zero/nonzero pairs reduce native calls; wrapped line
  starts remain kerning-free even when the pair exists in cache.
- [x] No final run/line width is stored in this family.
...
**Changes:**
- [x] Run P1 contract tests and churn fonts/generations/code points/misses/sizes/pairs/oversized
  admissions under cold, warm, clear, disabled, off-thread, and close scenarios.
- [x] Clear glyph, advance, and kerning families independently in all dependency-safe orders and
  verify immutable downstream primitive correctness/misses.
- [x] Reconcile logical/native/advance/kerning counters and retained weight with P2/M3 owners.

**Acceptance Checks:**
- [x] Each family stays under hard policy, emits exact diagnostics, and preserves M2 output/linear
  behavior disabled.
- [x] Negative caching is generation-safe and no cross-cache entry identity prevents independent clear.
