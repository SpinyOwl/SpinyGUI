...
**Changes:**
- [ ] Implement immutable keys from exact UTF-16 source, ordered semantic font identities/
...
- [ ] Store immutable primitives containing original UTF-16 boundaries, source/rendered code point,
...
**Acceptance Checks:**
- [ ] Same exact source/font/configuration with different widths/offsets shares the primitive value;
  source/font/order/size/configuration/generation/policy changes miss.
- [ ] Values are code-point safe, immutable, and contain no final line/run x/advance/list.
...
**Changes:**
- [ ] Route M2 measurement to consume cached/uncached identical primitive values, choose wrapping from
  current width/offset/policy, and build final immutable lines/runs once.
- [ ] Reset previous-pair contribution at explicit/wrapped line starts and across approved face
  boundaries even when pair/primitive values are warm.
- [ ] Rebase/freeze each final line's cumulative caret-advance array after wrapping/reset exactly as
  M2/P3; never derive it once globally and slice it across lines.
- [ ] Keep final run/glyph lists and line-specific advances out of the persistent primitive cache.

**Acceptance Checks:**
- [ ] One primitive hit supports multiple exact wrap widths/offsets with correct, independent final
  ranges/advances and line-start kerning reset.
- [ ] Warm/disabled paths return structurally equal M2 results and immutable public collections.
...
**Changes:**
- [ ] Run cold/warm/source churn/font-chain order/fallback/missing-replacement/supplementary/generation/
  large-value/oversized/clear/close/disabled scenarios.
- [ ] Assert logical resolutions/native primitive calls fall on warm hits for the intended reasons,
  while widths never create additional primitive entries.
- [ ] Clear chain/metrics/glyph/advance/kerning/primitive families independently in allowed orders and
  verify immutable value/key references remain correct and weight accounting is exact.

**Acceptance Checks:**
- [ ] Hard bounds/diagnostics/generation/disabled/UI-thread contracts pass and no stale hit crosses a
  semantic font change.
- [ ] Width churn leaves primitive entry count unchanged for otherwise equal keys.
