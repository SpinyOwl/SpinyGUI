...
- Review Outcome: Accepted — independent review of base `a1cfa607` and the recorded T1 source/test identities found no findings; focused artifact evidence and `git diff --check` were consistent
- Remaining Work: None
...
- Implemented Scope: Retained the previously accepted T6 example, documentation, native evidence, and initial-frame repair; added direct `valid -> 0x0 -> same valid` framebuffer recovery coverage plus render- and swap-failure injection that verifies propagation, publication boundaries, and reverse resource cleanup; reran focused, affected-module, analyzer, benchmark, and root-build gates
- Relevant Files and Symbols: `docs/features/lwjgl-application-host.md`; `README.md` (guide link); `spinygui.demo.simple/build.gradle.kts` (`runNavigationModalHostExample`); `spinygui.demo.simple/src/main/java/com/spinyowl/spinygui/demo/simple/NavigationModalHostExample.java`; `spinygui.demo.simple/src/test/java/com/spinyowl/spinygui/demo/simple/NavigationModalHostExampleTest.java`; `spinygui.core.backend.lwjgl.nanovg/src/main/java/com/spinyowl/spinygui/core/backend/renderer/lwjgl/AbstractLwjglApplication.java` (`run`, usable-framebuffer presentation tracking); `spinygui.core.backend.lwjgl.nanovg/src/test/java/com/spinyowl/spinygui/core/backend/renderer/lwjgl/AbstractLwjglApplicationFirstFrameTraceTest.java` (`sameFramebufferRendersAgainAfterTemporaryUnavailability`); `spinygui.core.backend.lwjgl.nanovg/src/test/java/com/spinyowl/spinygui/core/backend/renderer/lwjgl/AbstractLwjglApplicationTest.java` (`renderFailurePreventsPublicationAndSwapAndStillClosesResources`, `swapFailurePropagatesAfterRenderAndStillClosesResources`); `FlexLayoutTest`, `OverflowLayoutTest`, and `ParsedInlineWhitespaceLayoutTest` (current two-argument constructor migration)
- Acceptance Evidence:
  ...
  - Residual host-loop failure and recovery coverage (2026-09-02): Verified — Automated — the focused `AbstractLwjglApplicationTest` and `AbstractLwjglApplicationFirstFrameTraceTest` run passed 11 tests. The added cases prove a temporary unusable framebuffer forces the same valid size to render again, a render failure leaves paint unpublished and prevents swap, and render/swap failures propagate while owned resources still close in reverse order.
  - Fresh affected-module rerun (2026-09-02): Partially Verified — Automated — core passed 696 tests, LWJGL/NanoVG backend 133, facade 12, and simple demo 2 with zero failures, errors, or skips; complex-demo production compilation passed. `:spinygui.demo.complex:compileTestJava` remains blocked because the compiler cannot create output directories for `MainMenuExampleTest`.
  - Fresh aggregate analyzer and build rerun (2026-09-02): Blocked — Automated — `staticAnalysis --continue` and `build --continue` completed all available non-complex-demo analyzers and tests without findings or test failures. Both runs failed only because the locked/unwritable `spinygui.demo.complex/build` tree prevents `pmdMain` and `spotbugsMain` from creating report directories and `compileTestJava` from creating class directories; the isolated complex-demo `clean` task reproduced inability to delete that generated tree.
  - Fresh opt-in benchmark rerun (2026-09-02): Verified — Automated — `:spinygui.benchmark:test --rerun-tasks` passed 122 tests with zero failures, errors, or skips.
...
- Review Outcome: Not Reviewed — reopened only for the requested residual failure/recovery tests and fresh aggregate verification
- Remaining Work: Resolve the unrelated lock/access failure under `spinygui.demo.complex/build`, then rerun the complex-demo test/analyzer tasks and root aggregate gates. No additional E2 implementation or focused test work remains.
- Resume or Closure: Resume at the complex-demo generated-directory environment repair and aggregate rerun; keep E2 complete with verification caveat until those default-path gates pass and the reopened test change is reviewed.
