...
- Event tests prove aggregation is conservative across mixed batches: one full-refresh-required or
  unknown event makes the whole batch full-refresh-required.
- Same-target pointer and unused keyboard scenarios stop requesting full refresh.
- Hover enter/exit, press/release, click, focus, drag, selection, scrollbar, wheel, text editing,
  shortcut, and arbitrary listener scenarios remain force-full-equivalent.
- Counters expose unchanged, known-effect, and unknown-fallback decisions without per-event logging.
- Matched capped and uncapped recordings report input events, refresh requests, allocation per frame,
