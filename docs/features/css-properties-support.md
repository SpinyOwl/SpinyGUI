...
| `grid`, `grid-area`, `grid-auto-columns`, `grid-auto-flow`, `grid-auto-rows`, `grid-column`, `grid-column-end`, `grid-column-gap`, `grid-column-start`, `grid-gap`, `grid-row`, `grid-row-end`, `grid-row-gap`, `grid-row-start`, `grid-template`, `grid-template-areas`, `grid-template-columns`, `grid-template-rows` | Implemented subset; E3 in progress | Typed values and a dedicated formatter cover fixed/percentage/auto/fr/minmax/fit-content/repeat tracks, gaps, explicit placement, named areas, row/column auto-flow, dense packing, item stretch/alignment, nested grids, scroll metrics, and demo coverage. Deferred or requiring more proof: container `justify-content`/`align-content`, complete intrinsic sizing, named-line and `span <name>` semantics, subgrid, masonry, baseline alignment, negative line indexes, advanced shorthand forms, control/text interaction coverage, and renderer/manual verification. |
