...
| [E5: Text Performance Improvements](work/E5%20-%20Text%20performance%20improvements.md) | Improve text-path measurement, font lifecycle, controls, rendering submission, caches, and orchestration. | In progress | M1 evidence repair is complete. M2 has an approved measurement contract and a previously landed narrow append-only run optimization, but its documentation/fixtures and linear end-to-end measurement path remain open; M3-M8 are planned. |
